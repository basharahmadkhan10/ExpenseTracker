...
import { 
  Check, Phone, Mail, MapPin, Code, DollarSign, Database, FileSpreadsheet, Eye, 
  ArrowRight, Sparkles, AlertCircle, Sun, Moon
} from 'lucide-react';
...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen bg-dots-grid selection:bg-[#f5bb1b] selection:text-black">
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            <span className="text-lg font-black tracking-widest text-white uppercase font-mono">
              EXPENSE <span className="text-black bg-[#f5bb1b] border-2 border-black px-2 py-0.5 rounded shadow-[2px_2px_0px_#000]">TRACKER</span>
            </span>
          </div>
            ...
            {[
              ...
              <button
                ...
                className={`transition duration-200 hover:text-[#f5bb1b] cursor-pointer ${
                  activeNav === link.id ? 'text-[#f5bb1b] border-b-2 border-[#f5bb1b] pb-0.5' : 'text-slate-300'
                }`}
              >
                {link.label}
              </button>
      ...
      <section id="home" className="bg-[#1b1b1b] text-white pt-20 pb-24 relative overflow-hidden border-b-4 border-black">
        ...
        <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center relative z-10">
          ...
          <div className="space-y-8">
            ...
            <p className="text-slate-350 text-sm font-medium leading-relaxed max-w-xl">
              No more messy spreadsheets, conversion confusion, or time-travel membership calculations. We construct an elegant, auditable, and completely transparent picture of your shared flatmate ledger.
            </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {[
                ...
                <div key={idx} className="flex items-center gap-3">
                  ...
                  <span className="text-xs font-bold text-slate-200 uppercase tracking-wide">{item}</span>
                </div>
              ))}
            </div>
        ...
        <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-12 gap-16 items-center">
          ...
          <div className="lg:col-span-7 space-y-8">
            ...
            <p className="text-sm font-semibold leading-relaxed">
              The Shared Expense Tracker is a professional audit utility that prevents standard calculation disputes. It resolves members join/departure dynamically and registers all historical currency values transparently.
            </p>
            ...
            <div className="space-y-5">
              <div className="flex gap-4">
                ...
                <div>
                  <h4 className="text-xs font-black uppercase tracking-wider">Time-Travel Ledger</h4>
                  <p className="text-slate-500 dark:text-slate-400 text-xs font-medium mt-1 leading-relaxed">
                    Splits are computed according to active memberships on the exact date of the transaction. Late-joining members are excluded from early bills automatically.
                  </p>
                </div>
              </div>
              ...
              <div className="flex gap-4">
                ...
                <div>
                  <h4 className="text-xs font-black uppercase tracking-wider">Multi-Currency Accountability</h4>
                  <p className="text-slate-500 dark:text-slate-400 text-xs font-medium mt-1 leading-relaxed">
                    USD expenditures record the actual exchange rate at import time. This provides auditable splits and eliminates dynamic rate conversion discrepancies.
                  </p>
                </div>
              </div>
              ...
              <div className="flex gap-4">
                ...
                <div>
                  <h4 className="text-xs font-black uppercase tracking-wider">Interactive Review Queue</h4>
                  <p className="text-slate-500 dark:text-slate-400 text-xs font-medium mt-1 leading-relaxed">
                    Flagged anomalies (negatives, duplicates, unknown members, date issues) are placed in the review queue for manual correction, approval, or rejection.
                  </p>
                </div>
              </div>
            </div>
        ...
        <div className="max-w-7xl mx-auto px-6 flex flex-col md:flex-row justify-around items-center gap-8">
          {[
            ...
            <div key={idx} className="text-center space-y-1">
              <h3 className="text-5xl font-black tracking-tight border-2 border-black bg-white px-5 py-2 rounded-2xl shadow-[3px_3px_0px_#000] inline-block">{stat.num}</h3>
          ...
          <div className="neobrutal-card-white overflow-hidden">
            <table className="w-full text-left text-xs">
              ...
              <tbody className="divide-y-2 divide-black text-[11px] font-semibold">
                {[
                  { date: '15-Apr-2026', title: 'Sam Joined Group (Mid-Month Milestone)', desc: 'Membership updated. Excluded from early April calculations.', status: 'COMPLETED', val: '0.00 INR', type: 'system' },
                  { date: '12-Apr-2026', title: 'Rohan paid USD Rent (Converted)', desc: 'Original: $50.00 @ ₹83.50/$ rate. Audited transparently.', status: 'CONVERTED', val: '4,175.00 INR', type: 'expense' },
                  { date: '08-Apr-2026', title: 'Meera flagged Duplicate cost (Oversight)', desc: 'Duplicate laundry row parsed. Parked in review queue.', status: 'ANOMALY PENDING', val: '350.00 INR', type: 'anomaly' },
                  { date: '02-Apr-2026', title: 'Aisha settled Rohan (Ledger balanced)', desc: 'Debt minimization completed. Settlement logged.', status: 'SETTLED', val: '1,200.00 INR', type: 'settlement' }
                ].map((row, idx) => (
                  <tr key={idx} className="hover:bg-slate-50/50 dark:hover:bg-neutral-800/50 transition">
                    <td className="px-6 py-4 font-bold text-slate-400">{row.date}</td>
                    <td className="px-6 py-4">
                      <span className="block text-slate-900 dark:text-white font-extrabold">{row.title}</span>
                      <span className="text-[10px] text-slate-500 dark:text-slate-400 font-medium leading-normal block mt-0.5">{row.desc}</span>
                    </td>
                    <td className="px-6 py-4">
                      <span className={`px-2.5 py-1 rounded-lg text-[9px] font-black border-2 border-black shadow-[1.5px_1.5px_0px_#000] tracking-wider ${
                        row.status === 'COMPLETED' ? 'bg-slate-100 text-slate-700' :
                        row.status === 'CONVERTED' ? 'bg-sky-100 text-sky-850' :
                        row.status === 'ANOMALY PENDING' ? 'bg-amber-100 text-amber-850 animate-pulse' :
                        'bg-emerald-100 text-emerald-850'
                      }`}>
                        {row.status}
                      </span>
                    </td>
                    <td className={`px-6 py-4 text-right font-black text-xs ${row.type === 'expense' ? 'text-red-500' : row.type === 'settlement' ? 'text-emerald-500' : 'text-slate-900 dark:text-white'}`}>
                      {row.val}
                    </td>
                  </tr>
                ))}
      ...
      <section id="contact" className="py-24">
        <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-12 gap-16 items-start">
          ...
          <div className="lg:col-span-4 space-y-8">
            ...
            <p className="text-sm font-semibold leading-relaxed">
              If you discover calculation discrepancies or require custom schema configurations for your shared household, reach out to our core tech team. We resolve issues within 24 hours.
            </p>
            ...
            <div className="space-y-5">
              {[
                ...
                <div key={idx} className="flex gap-4">
                  ...
                  <div>
                    <h4 className="text-[9px] font-black text-slate-450 uppercase tracking-widest">{item.title}</h4>
          ...
          <div className="lg:col-span-8 neobrutal-card-white p-8">
            <div className="mb-6 space-y-1">
              <h3 className="text-xs font-black uppercase tracking-wider text-slate-900 dark:text-white">Send Free Message</h3>
              <p className="text-xs text-slate-500 dark:text-slate-400 font-semibold uppercase">Our admin team is standing by to assist with your ledger imports.</p>
            </div>
            ...
            {formSubmitted ? (
              <div className="rounded-2xl bg-emerald-50 border-3 border-black p-6 flex flex-col items-center justify-center text-center gap-3 shadow-[4px_4px_0px_#000]">
                ...
                <h4 className="text-xs font-black uppercase tracking-wider text-emerald-800">Message Dispatched Successfully!</h4>
                <p className="text-[10px] text-emerald-600 font-semibold uppercase leading-relaxed max-w-xs">We have queued your support request. A developer will review your split queries shortly.</p>
              </div>
            ) : (
              <form onSubmit={handleContactSubmit} className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <label className="block text-[9px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-1.5">Your Name</label>
                    <input 
                      type="text" 
                      required
                      value={contactName}
                  ...
                  <div>
                    <label className="block text-[9px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-1.5">Your Email</label>
                    <input 
                      type="email" 
                      required
                      value={contactEmail}
                ...
                <div>
                  <label className="block text-[9px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-1.5">Subject</label>
                  <input 
                    type="text" 
                    required
                ...
                <div>
                  <label className="block text-[9px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-1.5">Message</label>
                  <textarea 
                    rows={4}
                    required
      ...
      <footer className="bg-[#1b1b1b] text-white py-12 border-t-4 border-black">
        <div className="max-w-7xl mx-auto px-6 flex flex-col md:flex-row justify-between items-center gap-6">
          ...
          <div className="flex gap-4 text-xs font-bold uppercase tracking-wider text-slate-400">
            <Link href="/login" className="hover:text-[#f5bb1b] transition">Sign In</Link>
            <span className="text-neutral-800">|</span>
            <Link href="/dashboard" className="hover:text-[#f5bb1b] transition">Dashboard</Link>
          </div>
        </div>
      </footer>
    </div>
